 ...
 * Author: Jason Peng <jason@ufactory.cc>
 ...
 class VADERPlanner
 {
   public:
     ...
     void start();
     void stop();
     void addCollisionObjectForNonOperationalArm();
...
void VADERPlanner::init()
 {
   joint_names = group.getVariableNames();
...
void VADERPlanner::addCollisionObjectForNonOperationalArm()
 {
  moveit_msgs::CollisionObject collision_object;
  collision_object.header.frame_id = group.getPlanningFrame();

  collision_object.id = PLANNING_GROUP + "_collision_box";
  ; 

  shape_msgs::SolidPrimitive primitive;
  primitive.type = primitive.BOX;
  primitive.dimensions.resize(3);
  primitive.dimensions[0] = 0.2159;
  primitive.dimensions[1] = 0.2159;
  primitive.dimensions[2] = 0.2159;

  geometry_msgs::Pose box_pose;
  box_pose = group.getCurrentPose().pose; 
  ...
  // box_pose = current_pose.pose;
  
  box_pose.position.x += -0.0075;
  box_pose.position.y += 0.7104;
  box_pose.position.z -= -0.0245;
  ...
  ROS_INFO("Added collision object for non operational arm");
...
int main(int argc, char** argv)
 {
   ...
   plannerL.start();
   plannerR.start();
   plannerR.addCollisionObjectForNonOperationalArm();
