...
class VADERPlanner
{
  ...
  private:
    ...
    ros::ServiceServer plan_grasp_pose_srv;
...
void VADERPlanner::init()
{
  ...
  plan_grasp_pose_srv = node_handle.advertiseService("xarm_grasp_plan", &VADERPlanner::do_grasp_pose_plan, this);
...
//   cylinder_pose.position.y = msg_quat.position.y;
//   cylinder_pose.position.z = msg_quat.position.z;
//   // cylinder_pose.position.x = msg_quat.position.x - 0.2 * rotated_x_axis.x();
//   // cylinder_pose.position.y = msg_quat.position.y - 0.2 * rotated_x_axis.y();
//   // cylinder_pose.position.z = msg_quat.position.z - 0.2 * rotated_x_axis.z();
...
float CalcOptimalRotation(const tf::Vector3& fine_pose_axis, const tf::Vector3& gripper_lat_axis)
{
  // Calculate the angle between the two vectors
  float angle = fine_pose_axis.angle(gripper_lat_axis);

  // Calculate the rotation axis (cross product)
  tf::Vector3 rotation_axis = fine_pose_axis.cross(gripper_lat_axis);
  rotation_axis.normalize();

  // Calculate the optimal rotation angle
  float optimal_rotation = atan2(rotation_axis.y(), rotation_axis.x());
...
{ 
  ...
  if (found_ik) {
    ...
    try {
      tf_listener.lookupTransform("link_base", "vader_gripper_base_link", ros::Time(0), gripper_transform);
    }catch (tf::TransformException &ex) {
      ROS_ERROR("%s", ex.what());
      return false;
    }

    tf::Vector3 fine_pose_axis = msg_quat.orientation
    tf::Vector3 gripper_lat_axis = gripper_transform.getBasis().getColumn(0);


    double target_rotation = CalcOptimalRotation(fine_pose_axis, gripper_lat_axis);

  

    // double target_angle = joint_values[6] + M_PI/4;

    const auto* joint = joint_model_group -> getJointModels()[6];

    if (joint->satisfiesPositionBounds(&target_rotation)) {
      joint_values[6] = target_rotation;
      group.setJointValueTarget(joint_values);
    }

    moveit::planning_interface::MoveGroupInterface::Plan rot_plan;
    bool success = (group.plan(rot_plan) == moveit::planning_interface::MoveItErrorCode::SUCCESS);

    if (success){
      group.execute(plan);
      ROS_INFO("Rotated gripper to target angle");
    }
